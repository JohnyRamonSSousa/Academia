...
const DanceDetail: React.FC = () => {
        ...
        return (
            <div className="min-h-screen bg-zinc-950 flex items-center justify-center pt-24">
                <div className="text-center">
                    <h2 className="text-4xl font-black text-white mb-4 uppercase italic">DANÇA NÃO ENCONTRADA</h2>
    ...
    const goToPayment = () => {
        navigate('/payment', {
            state: {
                totalPrice: dance.priceSingleClass,
                basePrice: dance.priceSingleClass,
                planName: `Aula Avulsa: ${dance.name}`,
                isSingleClass: true
            }
        });
    };
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="mb-12">
                    <Link to="/crossfit" className="text-zinc-500 hover:text-lime-400 transition-colors uppercase font-black tracking-widest text-[10px] flex items-center gap-2">
                        <i className="fa-solid fa-arrow-left"></i> CrossFit & Dança
                    </Link>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                    ...
                    >
                        <img src={dance.image} alt={dance.name} className="absolute inset-0 w-full h-full object-cover" />
                        ...
                        <div className="absolute bottom-12 left-12 right-12">
                            <div className="w-16 h-16 bg-lime-400 rounded-2xl flex items-center justify-center text-black mb-6 shadow-2xl shadow-lime-400/20">
                                <i className={`${dance.icon} text-2xl`}></i>
                            </div>
                    ...
                    >
                        <div>
                            <h2 className="text-white text-sm font-black uppercase tracking-widest mb-6 flex items-center gap-3">
                                <span className="w-8 h-[1px] bg-lime-400"></span> Ritmo & Movimento
                            </h2>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800">
                                    ...
                                    <span className="text-white font-black uppercase italic">{dance.schedule}</span>
                                </div>
                                <div className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800">
                                    <span className="block text-zinc-500 text-[10px] uppercase font-bold mb-2 tracking-widest">Aula Avulsa</span>
                                    <span className="text-lime-400 font-black text-xl italic leading-none">R$ {dance.priceSingleClass.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h2 className="text-white text-sm font-black uppercase tracking-widest mb-6 flex items-center gap-3">
                                <span className="w-8 h-[1px] bg-lime-400"></span> Benefícios do {dance.name}
                            </h2>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {dance.benefits.map((benefit, i) => (
                        ...
                        <div className="pt-8 flex flex-col sm:flex-row gap-6">
                            <button
                                ...
                                className="flex-1 bg-white hover:bg-zinc-200 text-black px-12 py-6 rounded-2xl font-black uppercase tracking-widest transition-all transform hover:scale-105 shadow-xl shadow-white/5"
                            >
                                Comprar Aula
                            </button>
                            <Link
                ...
                {showCongratsModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
                        <motion.div
                            ...
                            className="bg-zinc-900 border border-zinc-800 rounded-[3rem] p-12 max-w-md w-full text-center shadow-3xl relative overflow-hidden"
                        >
                            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-lime-400 to-emerald-500"></div>

                            <div className="w-24 h-24 bg-lime-400 rounded-full flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-lime-400/20">
                                <i className="fa-solid fa-star text-black text-4xl animate-pulse"></i>
                            </div>

                            <h3 className="text-3xl font-black text-white italic uppercase mb-4 leading-tight">PARABÉNS PELA ESCOLHA!</h3>

                            <p className="text-zinc-400 mb-10 leading-relaxed text-sm">
                                Você está a um passo de transformar seu corpo e mente através da diversão. O <span className="text-white font-bold">{dance.name}</span> espera por você!
                            </p>

                            <div className="flex flex-col gap-4">
                                <button
                                    onClick={goToPayment}
                                    className="w-full neon-bg text-black font-black py-5 rounded-2xl uppercase tracking-[0.2em] hover:bg-lime-500 transition-all transform hover:scale-105"
                                >
                                    Ir para o Pagamento
                                </button>
                                <button
                                    onClick={() => setShowCongratsModal(false)}
                                    className="text-zinc-500 uppercase font-black tracking-widest text-[10px] hover:text-white transition-colors"
                                >
                                    Voltar
                                </button>
                            </div>
                ...
                <section className="mt-32">
                    <h4 className="text-white font-black uppercase italic text-2xl mb-12">Explore outros Ritmos</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {DANCES.filter(d => d.slug !== slug).slice(0, 3).map((other) => (
                            <Link key={other.id} to={`/dances/${other.slug}`} className="group relative rounded-3xl overflow-hidden aspect-video bg-zinc-900 border border-zinc-800 hover:border-lime-400/30 transition-all">
